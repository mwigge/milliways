...
- [x] `WorktreePool.Merge(task, strategy)`: open PRs or merge branches on task completion; clean up worktrees
...
- [x] Write unit tests: WorktreePool.Start creates correct number of worktrees; dependency ordering respected; cancel cleans up directories

## 3. State Checkpointing

- [x] Database migration: add `checkpoints` table (id, session_id, turn_n, messages JSON, tool_state JSON, ts)
- [x] Save checkpoint after every `turn_end` in daemon: serialise `session.Messages` → JSON → upsert row
- [x] On daemon start: detect sessions with `status = 'in_flight'`; load latest checkpoint; resume session
- [x] `session.rollback` RPC method: restore session to checkpoint at turn N; discard turns > N from history
...
- [x] `smj session export <session-id>` subcommand: write JSON cost lineage to stdout
- [x] `smj prices update [--file <path>]` subcommand: replace prices.toml from file or configured endpoint
- [x] Write unit tests: cost_usd computed correctly for known model; NULL for unknown model; aggregate across multiple turns
...
- [x] `ToolGate`: when session role has `bash = ["read"]` in agents.toml, call `BashArity` before bash execution; block write-arity commands with `ToolResult{error: "bash:write blocked for role review"}`
- [x] `review` role default: `bash = ["read"]` (can be overridden in agents.toml)
...
- [x] `smj workspace agents init` generates a starter `.smedja/agents.toml` from built-in defaults
