...
# GNU mktemp requires the X-template at the END (suffix needs --suffix=
# which BSD mktemp doesn't have). Generate a base name then rename.
_bundle_base="$(mktemp "${TMPDIR}/milliways-context-bundle-XXXXXX")"
BUNDLE_JSON="${_bundle_base}.json"
mv "$_bundle_base" "$BUNDLE_JSON"
