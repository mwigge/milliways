...
from mempalace.config import (  # noqa: E402
    MempalaceConfig,
    sanitize_kg_value,
    sanitize_name,
    sanitize_content,
...
from mempalace.backends.chroma import ChromaBackend, ChromaCollection  # noqa: E402
...
from .conversation import ConversationStore  # noqa: E402
...
# === CONVERSATION STORE (milliways fork addition) ===
...
def _get_conversation_store() -> ConversationStore:
    ...
    if _conversation_store is None:
        db_path = os.path.join(_config.palace_path, "conversations.sqlite3")
...
# ==================== CONVERSATION TOOLS (milliways fork) ====================


def tool_conversation_start(conversation_id: str, block_id: str = "", prompt: str = ""):
    """Start a new live conversation."""
    store = _get_conversation_store()
    return store.conversation_start(conversation_id, block_id, prompt)


def tool_conversation_end(conversation_id: str):
    """End a live conversation."""
    store = _get_conversation_store()
    return store.conversation_end(conversation_id)


def tool_conversation_get(conversation_id: str):
    """Get conversation details."""
    store = _get_conversation_store()
    result = store.conversation_get(conversation_id)
    if not result:
        return {"error": f"Conversation not found: {conversation_id}"}
    return result


def tool_conversation_list(limit: int = 20):
    """List recent conversations."""
    store = _get_conversation_store()
    return {"conversations": store.conversation_list(limit)}


def tool_conversation_append_turn(
    conversation_id: str, segment_id: str, role: str, provider: str, text: str
):
    """Append a turn to the conversation transcript."""
    store = _get_conversation_store()
    return store.turn_append(conversation_id, segment_id, role, provider, text)


def tool_conversation_start_segment(conversation_id: str, segment_id: str, provider: str):
    """Start a new segment (provider switch) within a conversation."""
    store = _get_conversation_store()
    return store.segment_start(conversation_id, segment_id, provider)


def tool_conversation_end_segment(segment_id: str, reason: str = ""):
    """End the current segment."""
    store = _get_conversation_store()
    return store.segment_end(segment_id, reason)


def tool_conversation_lineage(conversation_id: str):
    """Get the chain of segments in order."""
    store = _get_conversation_store()
    return {"conversation_id": conversation_id, "segments": store.lineage(conversation_id)}


def tool_conversation_events_append(
    conversation_id: str,
    block_id: str = "",
    segment_id: str = "",
    kind: str = "",
    ...
    store = _get_conversation_store()
    return store.event_append(conversation_id, block_id, segment_id, kind, provider, text, fields)


def tool_conversation_events_query(
    conversation_id: str, kind: str | None = None, since: str | None = None, limit: int = 100
):
    """Query runtime events for a conversation."""
    store = _get_conversation_store()
    return {"events": store.events_query(conversation_id, kind, since, limit)}


def tool_conversation_checkpoint(
    conversation_id: str,
    checkpoint_id: str,
    block_id: str = "",
    segment_id: str = "",
    ...
    store = _get_conversation_store()
    return store.checkpoint_save(
        conversation_id, checkpoint_id, block_id, segment_id, provider, reason, snapshot or {}
    )


def tool_conversation_resume(conversation_id: str, checkpoint_id: str = ""):
    """Resume from a checkpoint. If checkpoint_id empty, resumes from latest."""
    store = _get_conversation_store()
    if checkpoint_id:
        result = store.checkpoint_resume(conversation_id, checkpoint_id)
    else:
        result = store.checkpoint_latest(conversation_id)
    if not result:
        return {"error": f"No checkpoint found for conversation: {conversation_id}"}
    return result
...
TOOLS = {
    ...
    "mempalace_conversation_start": {
        "description": "Start a new live conversation tracking session.",
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": tool_conversation_start,
    },
    "mempalace_conversation_end": {
        "description": "End a live conversation.",
        "input_schema": {
            "type": "object",
            "properties": {"conversation_id": {"type": "string"}},
            "required": ["conversation_id"],
        },
        "handler": tool_conversation_end,
    },
    "mempalace_conversation_get": {
        ...
        "handler": tool_conversation_get,
    },
    "mempalace_conversation_list": {
        "description": "List recent conversations.",
        "input_schema": {
            "type": "object",
            "properties": {"limit": {"type": "integer", "default": 20}},
        },
        "handler": tool_conversation_list,
    },
    "mempalace_conversation_append_turn": {
        "description": "Append a turn to the conversation transcript.",
        "input_schema": {
            "type": "object",
            "properties": {
                "conversation_id": {"type": "string"},
            ...
            "required": ["conversation_id", "segment_id", "role", "provider", "text"],
        },
        "handler": tool_conversation_append_turn,
    },
    "mempalace_conversation_start_segment": {
        "description": "Start a new segment (provider switch) within a conversation.",
        "input_schema": {
            "type": "object",
            "properties": {
                "conversation_id": {"type": "string"},
            ...
            "required": ["conversation_id", "segment_id", "provider"],
        },
        "handler": tool_conversation_start_segment,
    },
    "mempalace_conversation_end_segment": {
        "description": "End the current segment.",
        "input_schema": {
            "type": "object",
            "properties": {
                "segment_id": {"type": "string"},
        ...
        "handler": tool_conversation_end_segment,
    },
    "mempalace_conversation_lineage": {
        "description": "Get the chain of segments in order for a conversation.",
        "input_schema": {
            "type": "object",
            "properties": {"conversation_id": {"type": "string"}},
            "required": ["conversation_id"],
        },
        "handler": tool_conversation_lineage,
    },
    "mempalace_conversation_events_append": {
        "description": "Append a runtime event to a conversation.",
        "input_schema": {
            "type": "object",
            "properties": {
                "conversation_id": {"type": "string"},
        ...
        "handler": tool_conversation_events_append,
    },
    "mempalace_conversation_events_query": {
        "description": "Query runtime events for a conversation.",
        "input_schema": {
            "type": "object",
            "properties": {
                "conversation_id": {"type": "string"},
        ...
        "handler": tool_conversation_events_query,
    },
    "mempalace_conversation_checkpoint": {
        "description": "Save a checkpoint of current conversation state.",
        "input_schema": {
            "type": "object",
            "properties": {
                "conversation_id": {"type": "string"},
            ...
            "required": ["conversation_id", "checkpoint_id"],
        },
        "handler": tool_conversation_checkpoint,
    },
    "mempalace_conversation_resume": {
        "description": "Resume from a checkpoint. If checkpoint_id is empty, resumes from the latest checkpoint.",
        "input_schema": {
            "type": "object",
            "properties": {
                "conversation_id": {"type": "string"},
        ...
        "handler": tool_conversation_resume,
    },
}
