...
// Conforms to OpenTelemetry GenAI semantic conventions:
...
const (
	...
	MetricTimeToFirstToken    = "gen_ai.client.operation.time_to_first_chunk"
	MetricTimePerOutputToken   = "gen_ai.client.operation.time_per_output_chunk"
)
...
// observeRequest pushes a single request count tick for GenAI operations.
func observeRequest(m MetricsObserver, agentID string) {
	if m == nil {
		return
	}
	m.ObserveCounter(MetricRequestCount, agentID, 1)
}

// observeOperationDuration records the total operation duration in seconds.
func observeOperationDuration(m MetricsObserver, agentID string, durationSeconds float64) {
	if m == nil {
		return
	}
