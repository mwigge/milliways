...
// writeReviewFile writes the combined review markdown to ~/.local/share/milliways/reviews/.
func writeReviewFile(root, label string, sections map[string]string, runnerOrder []string) (string, error) {
	home, _ := os.UserHomeDir()
	dir := filepath.Join(home, ".local", "share", "milliways", "reviews")
	if envDir := os.Getenv("MILLIWAYS_REVIEWS_DIR"); envDir != "" {
		dir = envDir
	}
