...
func formatObservabilityFrame(now time.Time, spans []observeRenderSpan, usage observeRenderUsage) string {
	...
	if limit := formatObserveTokenLimits(usage.Quotas); limit != "" {
...
// formatObserveTokenLimits returns a formatted string showing 24h/Week/Month token usage.
func formatObserveTokenLimits(quotas []observeRenderQuota) string {
	if len(quotas) == 0 {
		return ""
	}
	// Aggregate across all agents
	var dayUsed, weekUsed, monthUsed float64
	hasData := false
	for _, q := range quotas {
		if q.UsedDaily != nil {
			dayUsed += *q.UsedDaily
		...
		if q.UsedWeekly != nil && *q.UsedWeekly > 0 {
			weekUsed += *q.UsedWeekly
			hasData = true
		}
		if q.UsedMonthly != nil && *q.UsedMonthly > 0 {
			monthUsed += *q.UsedMonthly
			hasData = true
		}
	}
	// Only show if we have any data (even if 0, show to preserve visibility)
	if !hasData && dayUsed == 0 {
		return ""
	}
	dayStr := formatObserveTokenCount(int(dayUsed))
	weekStr := formatObserveTokenCount(int(weekUsed))
	monthStr := formatObserveTokenCount(int(monthUsed))
	return fmt.Sprintf("│   token limits:  24h %s / Week %s / Month %s\n", dayStr, weekStr, monthStr)
}
