...
func main() {
	...
	if sub == "workflow" {
		os.Exit(runWorkflow(rest, os.Stdout, os.Stderr))
	}
	if sub == "daemon" {
...
var observeAgents = []string{"minimax", "berget", "claude", "codex", "copilot", "kimi", "deepseek", "gemini", "local", "pool"}
...
func usage() {
	...
	fmt.Fprintln(os.Stderr, "  workflow list|show                         — inspect stored agent workflow graphs")
